    ...
    getProfile()
      .then((user) => {
        ...
        user.announcements.forEach((announcement) => {
          if (announcement.type.toLowerCase() === 'leilão') {
            setHaveAnnouncementAuction(true)
          }
          if (announcement.type.toLowerCase() === 'venda') {
            if (announcement.vehicle_type.toLowerCase() === 'carro') {
              setHaveAnnouncmentCarSale(true)
            }
            if (announcement.vehicle_type.toLowerCase() === 'moto') {
  ...
  return (
    <ProfileContainer>
      ...
      <div className='content'>
        <CreateAdCard isAdmin={user?.is_seller === true ? true : false} />
